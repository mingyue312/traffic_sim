...
def findindex(state):
    ...
    index = 0
    for i in range(0, 5):
        index += (3 ** (4 - i)) * (state[i] - 1)
    return index


def reward(state1, state2):
    "Given state 1 and state2 after an action was taken, calculates a reward used in Q-Learning formula"
    a = 1
    b = 1
    sumlanelength1 = 0
    sumlanelength2 = 0
    ...
    reward = a * (sumlanelength2 - sumlanelength1) + b * (state2[4] - state1[4])
    return reward
...
def UpdateQvalue(state1, state2, action):
    ...
    alpha = (1/float(n))
    gamma = 0.9
    # print(SATable[index1])
    # print(SATable[index1][action+1], reward(state1, state2), SATable[index1], SATable[index2])
    localoldqvalue = SATable[index2][action+1]
    SATable[index2][action + 1] = round(((1 - alpha) * (SATable[index2][action + 1]) + alpha * (reward(state1, state2) + \
                                  gamma * (max(SATable[index1][1], SATable[index1][2])))),10)
    localnewqvalue = SATable[index2][action+1]
    extraline = 1


def takeAction(state1):
    "Decide which action to take based on optimal Q-Value 90% of times, and pick randomly 10% of times"
    global SATable
    a = random.random()
    b = random.random()
    stateIndex = findindex(state1)
    if a <= 0.85:
        if SATable[stateIndex][1] > SATable[stateIndex][2]:
            return 0
        elif SATable[stateIndex][1] < SATable[stateIndex][2]:
            return 1
        else:
            if b <= 0.5:
                return 0
...
def takeActionoptimal(state):
    ...
    if SATableoptimal[stateindex][1]>SATableoptimal[stateindex][2]:
        return 0
    elif SATableoptimal[stateindex][1]<SATableoptimal[stateindex][2]:
        return 1
    elif SATableoptimal[stateindex][1] == SATableoptimal[stateindex][2]:
        a = random.random()
        if a < 0.5:
...
def qlearning(state_array):
    global prev_state
    global prev_action
    cur_state = ObserveState(state_array)
    action = takeAction(cur_state)
    if n != 1:
        UpdateQvalue(cur_state, prev_state, prev_action)
    prev_state = cur_state
    prev_action = action
    # print(SATable[100], "\n")
    return action

def optimalaction(state_array):
    cur_state = ObserveState(state_array)
...
# while (n < 1000):
#     state_array = randomarray()
#     qlearning(state_array)
